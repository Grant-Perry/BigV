...
///
/// Every stage is purely additive — new models plus defaulted fields on
/// `Ride` — so each is lightweight. They are still declared here explicitly:
/// a store that migrates through a named plan can never be stranded by a
/// silent inference SwiftData declines to make, and the next schema change
...
/// Cuts a ride into laps: `RideLap` and `RideClimbSplit` rows plus defaulted
/// counts on `Ride`, so V3 rides migrate untouched with no laps.
///
/// The current version references the live model classes. When V5 arrives,
/// freeze this version by giving it nested snapshot models the way V1 through
/// V3 do, and point the live classes at V5.
enum RideSchemaV4: VersionedSchema {

   static let versionIdentifier = Schema.Version(4, 0, 0)

   static var models: [any PersistentModel.Type] {
      [Ride.self, RideSample.self, RideRadarEvent.self, RideLap.self, RideClimbSplit.self]
   }
}
...
/// change again, or migration from a V3 store breaks.
